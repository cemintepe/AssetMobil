...
export default function StDashboard({ user, onLogout, onSelectCustomer }) {
  ...
  const fetchDealers = async () => {
    try {
      ...
      Alert.alert('Hata', 'Bayiler yüklenemedi');
    } finally {
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loadingCustomers ? (
        <View style={styles.centerMsg}>
          ...
          <Text style={styles.italicMsg}>Yükleniyor...</Text>
        </View>
        ...
        <View style={styles.centerMsg}>
          ...
          <Text style={styles.italicMsg}>
            {selectedDealer ? 'Müşteri bulunamadı.' : 'Lütfen önce bayi seçin.'}
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  italicMsg: { color: '#bbb', fontStyle: 'italic', fontSize: 14, marginTop: 10 },
